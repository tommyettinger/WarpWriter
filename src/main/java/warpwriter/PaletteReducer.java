...
public class PaletteReducer {
    ...
    /**
     * Constructs a default warpwriter.PaletteReducer that uses the standard RINSED palette.
     */
    public PaletteReducer() {
        exact(Coloring.RINSED);
    }

    /**
     * Constructs a warpwriter.PaletteReducer that uses the given array of RGBA8888 ints as a palette (see {@link #exact(int[])}
     * for more info).
     *
    ...
    /**
     * Constructs a warpwriter.PaletteReducer that uses the given array of Color objects as a palette (see {@link #exact(Color[])}
     * for more info).
     *
    ...
    /**
     * Constructs a warpwriter.PaletteReducer that analyzes the given Pixmap for color count and frequency to generate a palette
...
//        b2 = (b2 << 3 | b2 >>> 2);
    ...
    /**
     * Builds the palette information this warpwriter.PNG8 stores from the RGBA8888 ints in {@code rgbaPalette}, up to 256 colors.
     ...
     * this defaults to the standard RINSED palette, with 256 colors (including transparent).
     *
    ...
    /**
     * Builds the palette information this warpwriter.PNG8 stores from the Color objects in {@code colorPalette}, up to 256 colors.
     ...
     * has one color, then this defaults to the standard RINSED palette, with 256 colors (including transparent).
     *
     ...
     * one color, or limit is less than 2, then this defaults to the standard RINSED palette, with 256 colors (including
     * transparent).
     *
     ...
     * transparency). Because calling {@link #reduce(Pixmap)} (or any of warpwriter.PNG8's write methods) will dither colors that
     ...
     * value but instead stores the palette info in this object; a warpwriter.PaletteReducer can be assigned to the
     * {@link PNG8#palette} field or can be used directly to {@link #reduce(Pixmap)} a Pixmap.
     *
     * @param pixmap a Pixmap to analyze, making a palette which can be used by this to {@link #reduce(Pixmap)} or by warpwriter.PNG8
     */
     ...
     * instead stores the palette info in this object; a warpwriter.PaletteReducer can be assigned to the {@link PNG8#palette}
    ...
    /**
     * Modifies the given Pixmap so it only uses colors present in this warpwriter.PaletteReducer, dithering when it can.
     ...
     * compute the warpwriter.PaletteReducer once and reuse it, that will save some time.
    ...
    /**
     * Modifies the given Pixmap so it only uses colors present in this warpwriter.PaletteReducer, without dithering. This produces
    ...
    /**
     * Modifies the given Pixmap so it only uses colors present in this warpwriter.PaletteReducer, dithering when it can using
     ...
     * <br>
     * This method is not incredibly fast because of the extra calculations it has to do for dithering, but if you can
     * compute the warpwriter.PaletteReducer once and reuse it, that will save some time. Burkes dithering causes error to be
    ...
    /**
     * Modifies the given Pixmap so it only uses colors present in this warpwriter.PaletteReducer, dithering when it can using a
     ...
     * <br>
     * This method is not incredibly fast because of the extra calculations it has to do for dithering, but if you can
     * compute the warpwriter.PaletteReducer once and reuse it, that will save some time. This method is probably slower than
...
//                    xib2 = randomXi(state);
     ...
     * call this method; it will default to using the Rinsed palette otherwise.
