...
//                    lumas[i] = luma = curvedDouble();
                    ...
                    lumas[i] = luma = (0.375 * r + 0.5 * g + 0.125 * b) / 255.0;
...
//
        ...
        try {
            png8.writePrecisely(Gdx.files.local("Ripple"+PALETTE.length+".png"), pix, false);
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        try {
            png8.writePrecisely(Gdx.files.local("Ripple"+PALETTE.length+"_GLSL.png"), p2, false);
        } catch (IOException e) {
            e.printStackTrace();
        }

        int[][] RIPPLE_BONUS_RAMP_VALUES = new int[256][4];
        for (int i = 1; i < 64; i++) {
            int color = RIPPLE_BONUS_RAMP_VALUES[i | 128][2] = RIPPLE_BONUS_RAMP_VALUES[i][2] =
                    PALETTE[i];             
//            r = (color >>> 24);
//            g = (color >>> 16 & 0xFF);
//            b = (color >>> 8 & 0xFF);
            ...
            warm = warms[i];
            mild = milds[i];
            RIPPLE_BONUS_RAMP_VALUES[i | 64][1] = RIPPLE_BONUS_RAMP_VALUES[i | 64][2] =
                    RIPPLE_BONUS_RAMP_VALUES[i | 64][3] = color;
            RIPPLE_BONUS_RAMP_VALUES[i | 192][0] = RIPPLE_BONUS_RAMP_VALUES[i | 192][2] = color;
...
//            r = b + chromO;
            ...
            RIPPLE_BONUS_RAMP_VALUES[i | 192][1] = RIPPLE_BONUS_RAMP_VALUES[i | 128][1] =
                    RIPPLE_BONUS_RAMP_VALUES[i | 64][0] = RIPPLE_BONUS_RAMP_VALUES[i][1] =
            ...
            RIPPLE_BONUS_RAMP_VALUES[i | 192][3] = RIPPLE_BONUS_RAMP_VALUES[i | 128][3] =
                    RIPPLE_BONUS_RAMP_VALUES[i][3] =
                            MathUtils.clamp(r, 0, 255) << 24 |
                                    MathUtils.clamp(g, 0, 255) << 16 |
                                    MathUtils.clamp(b, 0, 255) << 8 | 0xFF;
            ...
            RIPPLE_BONUS_RAMP_VALUES[i | 128][0] = RIPPLE_BONUS_RAMP_VALUES[i][0] =
                    MathUtils.clamp(r, 0, 255) << 24 |
                            MathUtils.clamp(g, 0, 255) << 16 |
                            MathUtils.clamp(b, 0, 255) << 8 | 0xFF;
        }
        ...
        sb.append("private static final int[][] RIPPLE_BONUS_RAMP_VALUES = new int[][] {\n");
        for (int i = 0; i < 256; i++) {
            sb.append("{ 0x");
            StringKit.appendHex(sb, RIPPLE_BONUS_RAMP_VALUES[i][0]);
            StringKit.appendHex(sb.append(", 0x"), RIPPLE_BONUS_RAMP_VALUES[i][1]);
            StringKit.appendHex(sb.append(", 0x"), RIPPLE_BONUS_RAMP_VALUES[i][2]);
            StringKit.appendHex(sb.append(", 0x"), RIPPLE_BONUS_RAMP_VALUES[i][3]);
            sb.append(" },\n");
        ...
        PALETTE = new int[256];
        for (int i = 0; i < 64; i++) {
            System.arraycopy(RIPPLE_BONUS_RAMP_VALUES[i], 0, PALETTE, i << 2, 4);
        }
        sb.setLength(0);
        ...
        png8.palette = new PaletteReducer(PALETTE);
        try {
            png8.writePrecisely(Gdx.files.local("RippleBonus.png"), pix, false);
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        try {
            png8.writePrecisely(Gdx.files.local("RippleBonus_GLSL.png"), p2, false);
        } catch (IOException e) {
            e.printStackTrace();
        }
...
//            pix.drawPixel(i-1, 0, PALETTE[i]);
        ...
        png8.palette = new PaletteReducer(PALETTE);
        try {
            png8.writePrecisely(Gdx.files.local("RippleBonusMagicaVoxel.png"), pix, false);
        } catch (IOException e) {
            e.printStackTrace();
        }
