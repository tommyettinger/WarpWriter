...
public class PaletteReducer {
    ...
    public void exact(int[] rgbaPalette)
    {
        ...
        for (int i = 0; i < plen; i++) {
            color = rgbaPalette[i];
            paletteArray[count] = color;
            if(count == 0 || (color & 0x80) != 0)
                paletteMapping[(color >>> 17 & 0x7C00) | (color >>> 14 & 0x3E0) | (color >>> 11 & 0x1F)] = (byte) count++;
        }
        for (int r = 0; r < 32; r++) {
            for (int g = 0; g < 32; g++) {
                for (int b = 0; b < 32; b++) {
     ...
     * value but instead stores the palette info in this object; a PaletteReducer can be used directly to 
     ...
     * instead stores the palette info in this object; a PaletteReducer can be used directly to {@link #reduce(Pixmap)}
     * a Pixmap.
     * @param pixmap a Pixmap to analyze, making a palette which can be used by this to {@link #reduce(Pixmap)} or by PNG8
